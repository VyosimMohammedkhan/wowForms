...
async function findLablelsAndFieldtypes(page, url) {
    ...
    console.log('count: ', labelChildren.length, ' ', placeholders.length)
    ...
    for (label of labelChildren) {
        ...
        let isVisible = await isFieldVisible(page, fieldId, fieldnameattr)
        if (fieldTagname?.includes('SELECT')) {
            options = await getSelectOptions(page, fieldId, fieldnameattr)
        }

        fieldInfo = { fieldName, fieldTagname, inputType, isrequired, options } 

        console.log("field details",fieldName, isVisible, fieldInfo.inputType)
        if (isVisible && !fieldName.trim() == '' && !fieldTagname.trim() == '' && fieldInfo.inputType !== 'hidden' && !fieldsInfoSet.has(fieldkey)) {
    ...
    for (let placeholder of placeholders) {
        ...
        fieldInfo = { fieldName, fieldTagname, inputType, isrequired, options }
        if (isVisible && fieldInfo.fieldName && !fieldName.trim() == '' && fieldInfo.inputType !== 'hidden' && !fieldsInfoSet.has(fieldkey)) {
            fieldsInfoSet.add(fieldkey);
            fieldsInfoArray.push(fieldInfo);
        }

    }
    console.log('fieldSet:', fieldsInfoSet)
...
async function isFieldRequired(page, id, name) {
    ...
    const isRequired = await page.evaluate((id, name) => {
        ...
        if (fieldElement.hasAttribute('required') || fieldElement.classList.contains('required')) {
            return true;
        } else if (fieldElement.getAttribute('aria-required') === 'true') {
            return true;
        }
        return false;
    }, id, name);
...
async function isFieldVisible(page, id, name) {


    let isFieldVisible = await page.evaluate((id, name) => {
        ...
        console.log(JSON.stringify({ id, display, opacity, visibility, hiddenAttribute, disabledAttribute, fieldheight, fieldwidth }))
        if (
    ...
    if (isFieldVisible) {
        let invisibleAncestors = await page.$x(`//*[@id='${id}']//ancestor::*[contains(@style,'display: none')]`)//*/input[@name='first_name']
